...
namespace HoloLensWithOpenCVForUnity.UnityUtils.Helper
{
    ...
    public class HLCameraStreamToMatHelper : WebCamTextureToMatHelper
    {
        ...
        // This variable is being use in multi-threading.
        ...
        // This method will get triggered everytime a new frame is arrived.
        protected virtual void OnFrameSampleAcquired(VideoCaptureSample sample)
        {
            lock (latestImageBytesLockObject)
            {
                ...
                // copying the sample data in "_latestImageBytes". In way we are storing the frame
                sample.CopyRawImageDataIntoBuffer(_latestImageBytes);
            }

            // Extracting cameraToWorldMatrixAsFloat
            ...
            // Extracting projectionMatrixAsFloat
            ...
            // Extracting frame values from sample.
            ...
            // Initially it will be FALSE when first frame is acquired.
        ...
        protected override IEnumerator _Initialize()
        {
            ...
            // making this variable TRUE previosuly in "Initialize" method in parent class this variable was false
            isInitWaiting = true;
            ...
            // Making this variable TRUE
            isChangeVideoModeWaiting = true;

            // This is not NULL initially. ?? OR maybe initially it is null :/
            // That will explains calling the "StopVideoModeAsync" method below.
            // ASSUMING IT IS NULL IN THE START //
            if (videoCapture != null)
            {
                // Its strange but they are stoping the stopping 
                videoCapture.StopVideoModeAsync(result1 =>
                {
                    cameraParams = CreateCameraParams(videoCapture);

                    // starting video mode. If video mode is successfully started then it will return result2=TRUE
                    videoCapture.StartVideoModeAsync(cameraParams, result2 =>
                    {
                        // if StartVideoModeAsync is failed.
                        if (!result2.success)
                        {
                            isChangeVideoModeWaiting = false;
                            isInitWaiting = false;

                            // Cancelling the initCoroutine
                            CancelInitCoroutine();
            ...
            else
            {
                ...
                // Don't get confuse this videocapture below is the result of "CreateAync" method.
                HoloLensCameraStream.VideoCapture.CreateAync(videoCapture =>
                {
                    if (initCoroutine == null) return;

                    // checking if the result of "CreateAsync" is not NULL.
                    if (videoCapture == null)
                    {
                    ...
                    // The global variable "videoCapture" = videoCapture (which is the result of CreateAsync)
                    this.videoCapture = videoCapture;

                    //Request the spatial coordinate ptr if you want to fetch the camera and set it if you need to 
#if XR_PLUGIN_OPENXR
                    // setting worldOrigin
                    videoCapture.WorldOrigin = spatialCoordinateSystem;
#elif XR_PLUGIN_WINDOWSMR || BUILTIN_XR
                    ...
                    // creating parameters for the camera. This "CreateCameraParams" is custom method exist in this class.
                    cameraParams = CreateCameraParams(videoCapture);

                    // removing the method "OnFrameSampleAcquired" as subscriber of the event FrameSampleAcquired.
                    videoCapture.FrameSampleAcquired -= OnFrameSampleAcquired;
                    
                    // adding the method "OnFrameSampleAcquired" as subscriber of the event FrameSampleAcquired.
                    videoCapture.FrameSampleAcquired += OnFrameSampleAcquired;
                    
                    // Starting Video Capture at this point
                    videoCapture.StartVideoModeAsync(cameraParams, result =>
                    {
                        if (!result.success)
                        {
            ...
            // Initial frame count variable
            int initFrameCount = 0;
            bool isTimeout = false;

            while (true)
            {
                // We have assigne the value _timeoutFrameCount = 300;
                ...
                else if (didUpdateThisFrame)
                {
                    ...
                    if (onInitialized != null)
                        // Here we are invoking this event. It will call the "OnWebCamTextureToMatHelperInitialized" method in FaceDection class.
