...
namespace HoloLensWithOpenCVForUnityExample
{
    ...
    public class HoloOnlyRayCast : MonoBehaviour
    {
        ...
        public void OnFrameMatAcquired(Mat grayMat, Matrix4x4 projectionMatrix, Matrix4x4 cameraToWorldMatrix, CameraIntrinsics cameraIntrinsics)
        {
            ...
            if (enableDetection && !isDetecting)
            {
                ...
                System.Threading.Tasks.Task.Run(() =>
                {
                    ...
                    if (regionOfInterestSelected)
                    {
                        ...
                        Imgproc.cvtColor(grayMat4Thread, hsvMat, Imgproc.COLOR_RGB2HSV);

                        initialRect.x = (hsvMat.width() / 2) - (200 / 2);
                        initialRect.y = (hsvMat.height() / 2) - (100 / 2);
                        ...
                        //Debug.Log($"Inside if statement. regionOfInterestSelected : {regionOfInterestSelected} w: {centreOfFrame.width} h: {centreOfFrame.height}");
                    }
                    else
                    {
            ...
            if (!useSeparateDetection)
            {
                ...
                if(regionOfInterestSelected)
                {
                    DrawDownScaleFaceRects(grayMat, detectionResult.ToArray(), DOWNSCALE_RATIO, COLOR_WHITE, 6);
                }
                else
                {
                    Imgproc.circle(grayMat, new Point(grayMat.width() / 2, grayMat.height() / 2), 5, COLOR_WHITE, 4);
                }
            }
 /*           else
            {
